...
function getLuminance(color2) {
  color2 = decomposeColor(color2);
  var rgb2 = color2.type === "hsl" ? decomposeColor(hslToRgb(color2)).values : color2.values;
...
var StyleRule = /* @__PURE__ */ function(_BaseStyleRule) {
  ...
  _proto2.toString = function toString(options) {
    ...
    var opts2 = link ? _extends$1({}, options, {
      allowEmpty: true
    }) : options;
...
const rgb$1 = function rgbGamma(y) {
  ...
  function rgb2(start, end) {
    var r2 = color2((start = rgb$2(start)).r, (end = rgb$2(end)).r), g = color2(start.g, end.g), b = color2(start.b, end.b), opacity = nogamma$1(start.opacity, end.opacity);
    return function(t2) {
      start.r = r2(t2);
      start.g = g(t2);
      start.b = b(t2);
...
function string$1(a, b) {
  ...
  return s.length < 2 ? q[0] ? one$1(q[0].x) : zero$1(b) : (b = q.length, function(t2) {
...
function axis(orient2, scale) {
  ...
  function axis2(context) {
    var values = tickValues == null ? scale.ticks ? scale.ticks.apply(scale, tickArguments) : scale.domain() : tickValues, format2 = tickFormat2 == null ? scale.tickFormat ? scale.tickFormat.apply(scale, tickArguments) : identity : tickFormat2, spacing = Math.max(tickSizeInner, 0) + tickPadding, range2 = scale.range(), range0 = +range2[0] + offset, range1 = +range2[range2.length - 1] + offset, position = (scale.bandwidth ? center : number$1)(scale.copy(), offset), selection = context.selection ? context.selection() : context, path = selection.selectAll(".domain").data([null]), tick = selection.selectAll(".tick").data(values, scale).order(), tickExit = tick.exit(), tickEnter = tick.enter().append("g").attr("class", "tick"), line = tick.select("line"), text = tick.select("text");
...
function interpolate(a, b) {
  var t2 = typeof b, c;
  return b == null || t2 === "boolean" ? constant(b) : (t2 === "number" ? number : t2 === "string" ? (c = color(b)) ? (b = c, interpolateRgb) : string : b instanceof color ? interpolateRgb : b instanceof Date ? date : isNumberArray(b) ? numberArray : Array.isArray(b) ? genericArray : typeof b.valueOf !== "function" && typeof b.toString !== "function" || isNaN(b) ? object : number)(a, b);
}
function piecewise(interpolate$12, values) {
  if (values === void 0)
    values = interpolate$12, interpolate$12 = interpolate;
  var i = 0, n = values.length - 1, v = values[0], I = new Array(n < 0 ? 0 : n);
  while (i < n)
    I[i] = interpolate$12(v, v = values[++i]);
  return function(t2) {
  ...
  useEffect(() => {
    ...
    if (!considerSelections || obsColorEncoding === "geneSelection") {
      ...
      if (featureValueColormap && pointsVisible) {
        ...
        const x = linear$1().domain(scaledDataExtent).range([0.5, width - 0.5]);
        ...
        const tSize = 12;
        const xPercentile = linear$1().domain([0, 1]).range([tSize / 2, width - tSize / 2 - 2]);
        const axisTicks = g.append("g").attr("transform", `translate(0,${titleHeight + rectHeight + 15})`).style("font-size", "9px").call(axisBottom(xPercentile).tickValues(contourPercentiles).tickFormat(format(".0%")).tickSizeOuter(0));
        axisTicks.selectAll("line,path").style("stroke", foregroundColor);
        axisTicks.selectAll("text").style("fill", foregroundColor);
        ...
        if ((contourPercentages == null ? void 0 : contourPercentages[1]) - (contourPercentages == null ? void 0 : contourPercentages[0]) <= NEIGHBOR_THRESHOLD || (contourPercentages == null ? void 0 : contourPercentages[2]) - (contourPercentages == null ? void 0 : contourPercentages[1]) <= NEIGHBOR_THRESHOLD) {
          axisTicks.selectAll("text").attr("transform", (d, i) => `translate(0,${i === 0 || i === contourPercentiles.length - 1 ? 0 : 10})`);
        }
        const triangleGroupG = g.append("g").attr("transform", `translate(0,${titleHeight + rectHeight + 4})`);
        contourPercentiles.forEach((p, i) => {
          const triangleG = triangleGroupG.append("g").attr("transform", `translate(${xPercentile(p) - tSize / 2},0)`);
          triangleG.append("polygon").attr("points", `0 0, ${tSize} 0, ${tSize / 2} ${tSize * 85 / 100}`).style("opacity", (i + 0.5) / contourPercentiles.length);
        });
        const thresholdGroupG = g.append("g").attr("transform", `translate(0,${titleHeight + rectHeight})`);
        const thresholdFormatter = format(".0f");
        contourPercentiles.forEach((p, i) => {
          const contourThreshold = xMin + (xMax - xMin) * ((contourThresholds == null ? void 0 : contourThresholds[i]) / 255);
...
const parse$6 = (version, options) => {
  ...
  const r2 = options.loose ? re$1[t$1.LOOSE] : re$1[t$1.FULL];
  if (!r2.test(version)) {
    return null;
  }
  try {
    ...
    return photometricInterpretation !== "RGB" && spatialLayerColormap === null && channelCoordination && channelScopes ? /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        ...
        children: channelScopes.map((cScope) => {
          ...
          const imageWrapperInstance = (_b = (_a = images == null ? void 0 : images[layerScope]) == null ? void 0 : _a.image) == null ? void 0 : _b.instance;
          ...
          return spatialLayerVisible && spatialChannelVisible && spatialChannelLabelsVisible ? /* @__PURE__ */ jsxRuntimeExports.jsx(
            ...
            `${layerScope}-${cScope}-${channelIndex}-${rgbColor}`
          ) : null;
        })
      },
      layerScope
